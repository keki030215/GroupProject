...
server <- function(input, output) {
  ...
  output$plot_studyperformance <- renderPlotly({
    var1 <- input$select_relation
      if(var1 == "Math v.s. Daily Consumption"){
        performance_plot <- d3 %>% 
        ...
        if(var1 == "Math v.s. Weekend Consumption"){
          performance_plot <- d3 %>% 
            filter(school %in% input$select_school) %>%
            mutate(ave_grade = (G1.x + G2.x + G3.x)/3) %>% 
          ...
          if(var1 == "Portuguese v.s. Daily Consumption"){
            performance_plot <- d3 %>% 
              filter(school %in% input$select_school) %>%
            ...
            if(var1 == "Portuguese v.s. Weekend Consumption"){
              performance_plot <- d3 %>% 
                filter(school %in% input$select_school) %>%
                mutate(ave_grade = (G1.y + G2.y + G3.y)/3) %>% 
